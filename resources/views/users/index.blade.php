                    ...
                    <tr class="text-xs hover:bg-blue-50/20 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('users.edit', $u) }}" class="p-2 bg-blue-50/30 border border-blue-100/60 text-blue-500 hover:text-white hover:bg-brand-light rounded-xl transition-all btn-premium">
                                ...
                                @if ($u->id !== auth()->id())
                                    <form action="{{ route('users.destroy', $u) }}" method="POST" onsubmit="return window.confirmSubmit(event, 'Apakah Anda yakin ingin menonaktifkan user ini?', 'Nonaktifkan Pengguna', 'danger')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 bg-blue-50/30 border border-blue-100/60 text-slate-400 hover:text-red-600 hover:bg-red-50 hover:border-red-100 rounded-xl transition-all btn-premium">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
