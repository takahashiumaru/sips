...
@section('page_subtitle', 'Perbarui informasi personal atau data akademik siswa')
...
<div class="max-w-2xl bg-white rounded-2xl border border-blue-100/50 p-6 shadow-sm card-premium">
    <div class="flex items-center justify-between border-b border-blue-50 pb-4 mb-6">
        <h2 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Form Edit Siswa</h2>
        <a href="{{ route('siswa.index') }}" class="text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors flex items-center gap-1">
    ...
    <form action="{{ route('siswa.update', $siswa) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select name="wali_murid_id" id="wali_murid_id"
                    ...
                    @foreach ($waliMurid as $w)
                        <option value="{{ $w->id }}" {{ old('wali_murid_id', $siswa->wali_murid_id) == $w->id ? 'selected' : '' }}>
                            {{ $w->name }} ({{ $w->email }})
                        </option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <textarea name="alamat" id="alamat" rows="3"
                class="block w-full px-4 py-2.5 bg-blue-50/30 border border-blue-100/60 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-light/20 focus:border-brand-light focus:bg-white transition-all text-xs font-semibold form-input-premium"
                placeholder="Masukkan alamat tinggal siswa saat ini">{{ old('alamat', $siswa->alamat) }}</textarea>
        </div>
