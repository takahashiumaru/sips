...
@section('page_title', 'Perbarui Tagihan SPP')
@section('page_subtitle', 'Ubah rincian nominal atau tanggal jatuh tempo tagihan')
...
<div class="max-w-2xl bg-white rounded-2xl border border-blue-100/50 p-6 shadow-sm card-premium">
    <div class="flex items-center justify-between border-b border-blue-50 pb-4 mb-6">
        <h2 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Form Edit Tagihan</h2>
    ...
    <form action="{{ route('tagihan.update', $tagihan) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')

        <div class="p-4 bg-blue-50/20 border border-blue-100/50 rounded-xl space-y-2.5 mb-2">
            <div class="flex justify-between text-xs">
                <span class="text-blue-500/80 font-bold uppercase tracking-wider text-[9px]">Nama Siswa</span>
                <span class="text-slate-800 font-bold">{{ $tagihan->siswa->nama_lengkap ?? '-' }}</span>
            </div>
            <div class="flex justify-between text-xs">
                <span class="text-blue-500/80 font-bold uppercase tracking-wider text-[9px]">Kelas</span>
                <span class="text-slate-800 font-bold">{{ $tagihan->siswa->kelas->nama_kelas ?? '-' }}</span>
            </div>
            <div class="flex justify-between text-xs">
                <span class="text-blue-500/80 font-bold uppercase tracking-wider text-[9px]">Periode Tagihan</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="jumlah_tagihan" id="jumlah_tagihan" value="{{ old('jumlah_tagihan', $tagihan->jumlah_tagihan) }}" required min="0" step="1"
            ...
            <div>
                ...
                <input type="date" name="jatuh_tempo" id="jatuh_tempo" value="{{ old('jatuh_tempo', $tagihan->jatuh_tempo ? $tagihan->jatuh_tempo->format('Y-m-d') : '') }}" required
        ...
        <div>
            ...
            <textarea name="catatan" id="catatan" rows="2"
                ...
                placeholder="Tambahkan catatan jika diperlukan">{{ old('catatan', $tagihan->catatan) }}</textarea>
        </div>
